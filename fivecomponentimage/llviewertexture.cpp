...
// 			<< mID.getString() << LL_ENDL;
	...
	U8 raw_components = mRawImage->getComponents();             // number of components (channels) in image 
 	if (raw_components < 1 || raw_components > 4)               // validate number of components.
 	{
 		// Number of image components is invalid. 1=greyscale, 2=greyscale with alpha, 3=RGB, 4=RGBA. 5 has been seen, and crashes the texture reader.
 		LL_INFOS() << "Invalid number of image components: (" << (U32)raw_components << ")" << LL_ENDL;
 		size_okay = false;
 	}
